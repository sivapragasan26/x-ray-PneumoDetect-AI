...
if "prediction_results" in st.session_state and st.session_state["prediction_results"] is not None:
    ...
    if not prediction_data["success"]:
        ...
        with st.container(border=True):
            ...
            # 1. DIAGNOSIS CONTAINERS (your existing code)
...
if res["diagnosis"] == "PNEUMONIA":
    ...
    <div style="background:rgba(255,0,0,0.1);border:1px solid rgba(255,0,0,0.3);
        ...
        <p style="color:#ffffff;margin-bottom:8px;"><strong>Confidence:</strong>
           {res['confidence_level']} ({res['confidence']}%)</p>
        <p style="color:#ffffff;margin-bottom:20px;"><strong>Recommendation:</strong>
           {res['recommendation']}</p>
        <div style="background-color:rgba(255,255,255,0.2);border-radius:8px;height:12px;
                    overflow:hidden;margin-bottom:8px;">
            <div style="background-color:#d32f2f;height:100%;width:{res['confidence']}%;
    ...
    st.markdown(f"""
    <div style="background:rgba(0,255,0,0.1);border:1px solid rgba(0,255,0,0.3);
                border-radius:12px;padding:20px;margin-bottom:20px;">
        ...
        <div style="background-color:rgba(255,255,255,0.2);border-radius:8px;height:12px;
                    overflow:hidden;margin-bottom:8px;">
            <div style="background-color:#388e3c;height:100%;width:{res['confidence']}%;
                        border-radius:8px;transition:width .5s ease;"></div>
        </div>
        <div style="text-align:center;color:#ffffff;font-size:13px;font-weight:500;">
            {res['confidence']}% Confidence Level
