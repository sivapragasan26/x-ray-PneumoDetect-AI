...
def load_external_dataset(dataset_path):
    """
    Load external validation dataset from specified directory structure.
    
    Args:
        dataset_path (str): Path to external_validation_dataset/test/
        
    Returns:
        tuple: (X_external, y_external) - Images and labels as numpy arrays
    """
    ...
    print(f"Loaded {len(images)} external validation samples")
...
def generate_comprehensive_visualizations(y_true, y_pred, y_prob, metrics):
    """Generate comprehensive visualization suite for external validation."""
    ...
    plt.title('External Validation - Enhanced Confusion Matrix', fontsize=16, fontweight='bold')
    ...
    plt.title('External Validation - ROC Curve', fontsize=16, fontweight='bold')
    ...
    plt.title('External Validation - Precision-Recall Curve', fontsize=16, fontweight='bold')
    ...
    bars2 = plt.bar(x_pos + width/2, external_vals, width, label='External Validation', 
    ...
    plt.title('Internal vs External Validation - Performance Comparison', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('External Dataset - Class Distribution', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('External Dataset - Class Proportion', fontsize=14, fontweight='bold')
    ...
    plt.title('External Validation - Prediction Confidence Distribution', fontsize=16, fontweight='bold')
    plt.legend(fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    ...
    try:
        ...
        plt.title('External Validation - Calibration Plot', fontsize=16, fontweight='bold')
        plt.legend(fontsize=12)
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
    ...
    plt.suptitle('External Validation - Comprehensive Metrics Dashboard', 
                 fontsize=20, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.savefig('8_comprehensive_metrics_dashboard.png', dpi=300, bbox_inches='tight')
    plt.close()

def evaluate_external_validation(model_path, external_data_path):
    """
    Comprehensive external validation evaluation with metrics and visualizations.
    
    Args:
        model_path (str): Path to trained model file
        external_data_path (str): Path to external validation dataset
    ...
    X_external, y_external = load_external_dataset(external_data_path)
    
    # Generate predictions
    print("Running predictions on external dataset...")
    predictions_prob = model.predict(X_external)
    predictions_binary = (predictions_prob > 0.5).astype(int).flatten()
    
    # Calculate comprehensive metrics
    metrics = calculate_clinical_metrics(y_external, predictions_binary)
    roc_auc = roc_auc_score(y_external, predictions_prob.flatten())
    
    # Print results
    print("\n" + "="*60)
    print("COMPREHENSIVE EXTERNAL VALIDATION RESULTS")
    print("="*60)
    
    print(f"Dataset Size: {len(y_external)} samples")
    ...
    print(f"{'Sample Size':<15} {internal_results['dataset_size']:<12} {len(y_external):<12} {len(y_external) - internal_results['dataset_size']:<+12}")
    ...
    generate_comprehensive_visualizations(y_external, predictions_binary, predictions_prob, metrics)
    ...
    all_metrics = {
        ...
        'external_precision': metrics['precision'],
        'external_f1': metrics['f1'],
        'external_roc_auc': roc_auc,
        'external_mcc': metrics['mcc'],
        'external_kappa': metrics['kappa'],
        'external_ppv': metrics['ppv'],
        'external_npv': metrics['npv'],
        'external_fpr': metrics['fpr'],
        'external_fnr': metrics['fnr'],
    ...
    results_df.to_csv('comprehensive_external_validation_results.csv', index=False)
    
    # Generate classification report
    class_report = classification_report(y_external, predictions_binary, 
    ...
    print("   comprehensive_external_validation_results.csv")
...
if __name__ == "__main__":
    ...
    external_data_path = "../external_validation_dataset/test"
    
    # Execute external validation
    results = evaluate_external_validation(model_path, external_data_path)
